...
* 其中若要配置 b 中的相关参数需要在 upstream 表中的 postback_pattern 字段配置对应内容；若下游想要回传其他从上游/三方得到的安装信息，也可以在 upstream 表中的 postback_pattern 字段配置，前提是上游/三方支持回传该参数信息。详细配置规则将在 3.2.x.1 中说明。
...
* 其中若要配置 c 中的相关参数需要在 upstream 表中的 postback_event_pattern 字段配置对应内容；若下游想要回传其他从上游/三方得到的安装信息，也可以在 upstream 表中的 postback_event_pattern 字段配置，前提是上游/三方支持回传该参数信息。详细配置规则将在 3.2.x.2 中说明。
...
* 配置该部分需要与 2.2.1 节配置的 postback_url 对齐。例如，postback_url 的查询参数需要回传{breason}宏参，这里配置时就必须回调{breason}。
